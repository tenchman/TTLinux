...
default_all(void)
{
	...
	default_line("cache_log /var/logs/cache.log");
	default_line("cache_store_log /var/logs/store.log");
...
#endif
	default_line("pid_filename /var/logs/squid.pid");
	...
	default_line("error_directory /usr/share/squid/errors/English");
...
defaults_if_none(void)
{
	...
	if (check_null_cachedir(Config.cacheSwap)) {
		default_line("cache_dir ufs /var/cache 100 16 256");
	}
#if USE_WCCPv2
